<template>
  <header v-show="isVisible" id="header" :class="[
    'header',
    ...
    <div v-if="accountData" class="header__ether">
      <div class="header__ether-img">
        <img style="height: 46px" :src="coins[accountData?.currentBaseCurrency].logo.length > 0
    ...
    <div class="header__etc">
      <div class="header__etc-middle">
        <div v-if="accountData" class="middle-list__title">
          ...
          <ul class="header__links">
            ...
            <li v-for="link in getLinkInfo(accountData.currentBaseCurrency)" :key="link.href"
              class="header__links-item">
              <a :href="link.href" rel="noopener noreferrer nofollow" target="_blank" class="header__links-link">{{
                link.title }}</a>
            </li>
          </ul>
        </div>
      ...
      <div class="header__author-wrap">
        <router-link v-if="isAuthorized" to="/wallet/deposit/USDT" class="header-deopsit-btn">
          {{ $t("common.deposit") }}
        </router-link>
        <router-link v-if="!isAuthorized" to="/login" class="header__login-no-auth">
          <div class="burger">
            <span class="burger__text">{{ $t("common.login") }}</span>
          </div>
        </router-link>
        <div v-if="isAuthorized" v-click-outside="closeSettingMenu" class="header__author"
          @click="toggleIsSettingsOpen">
          <svg class="header__author-img" width="20" height="20" viewBox="0 0 24 24" fill="none"
          ...
          <svg class="header__author-image" width="10" height="10" viewBox="0 0 21 21"
            xmlns="http://www.w3.org/2000/svg" @click.stop="toggleIsSettingsOpen">
            <g id="" transform="translate(-254 -232)">
              <rect data-name="Rectangle 1" width="21" height="21" transform="translate(254 232)"
          ...
          <ul v-if="isAuthorized" class="author-list" :class="{ 'author-list-open': isOpenSettingsMenu }">
            <template v-for="item in authorizedMenuItems">
              <li v-if="RegExp('^https?://|^//').test(item.path)" :key="item.key" class="author-list__item">
                <a :href="item.path" rel="noopener noreferrer nofollow" :target="item.target" class="author-list__link">
              ...
              <router-link v-else :to="item.path" class="author-list__item">
                <a class="author-list__link">
                  <svg v-if="item.iconClass === 'cog'" class="author-list__link-icon" width="20" height="20"
                  ...
                  <svg v-if="item.iconClass === 'headphones'" class="author-list__link-icon" width="20" height="20"
                    viewBox="0 0 1792 1792" xmlns="http://www.w3.org/2000/svg">
                    <path
            ...
            <li class="author-list__item">
              <a class="author-list__link" @click="handleLogout()">
                <svg class="author-list__link-icon" width="20" height="20" viewBox="0 0 1792 1792"
                  xmlns="http://www.w3.org/2000/svg">
                  <path
      ...
      <MenuComponent />
      <LanguageSelector />
      <div class="themeSelector" :class="{ dark: theme === 'dark' }" @click="changeTheme">
